...
function inicio(){
	...
	quica = new Quica();
	calacas = [new Calaca(65), new Calaca(140),
				   new Calaca(185), new Calaca(260),
				   new Calaca(305), new Calaca(380)];
	run();	
...
function run(){ 
	...
	if(jugando){  
		...
		contextoBuffer.fillText("GAMEOVER", 180, 200);
		contextoBuffer.fillStyle = "#ffffff";
		contextoBuffer.font = "15px sans-serif";
		contextoBuffer.fillText("El pollito a muerto T.T", 250, 220);
		contexto.clearRect(0,0,miCanvas.width,miCanvas.height);
		contexto.drawImage(buffer, 0, 0);
	}
